...
(() => {
  ...
  var GoogleTranslationProvider = class {
    apiKey;
    constructor() {
    ...
    async translate(text, targetLanguage, sourceLanguage) {
      try {
        ...
        return {
          ...
          sourceLanguage: sourceLanguage || response.data.data.translations[0].detectedSourceLanguage,
          targetLanguage,
          service: "google"
        };
      } catch (error) {
  ...
  var DeepLTranslationProvider = class {
    apiKey;
    constructor() {
    ...
    async translate(text, targetLanguage, sourceLanguage) {
      try {
        ...
        return {
          ...
          sourceLanguage: sourceLanguage || response.data.translations[0].detected_source_language,
          targetLanguage,
          service: "deepl"
        };
      } catch (error) {
  ...
  var OpenAITranslationProvider = class {
    apiKey;
    constructor() {
        ...
        const response = await axios_default.post(
          "https://api.openai.com/v1/chat/completions",
          {
            ...
            messages: [
              {
                role: "system",
                content: `You are a professional translator. Translate the following text from ${sourceLanguage || "auto-detected language"} to ${targetLanguage}. Only return the translated text without any explanations or additional content.`
              },
              {
                role: "user",
                content: text
  ...
  var TranslationServiceFactory = class {
    static createProvider(service) {
      switch (service) {
        case "google":
  ...
  var ContentScript = class {
    ...
    constructor() {
      ...
      this.state = {
        isEnabled: false,
        isTranslating: false,
        translatedElements: /* @__PURE__ */ new Set(),
    ...
    async handleMutations(mutations) {
      if (!this.config.enabled || this.state.isTranslating)
        return;
    ...
    async translateElement(element) {
      if (this.state.translatedElements.has(element))
        return;
